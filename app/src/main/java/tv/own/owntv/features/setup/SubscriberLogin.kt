...
class SubscriberLoginClient(
     ...
     *
     * [Status.rev] بصمة تجمع شقّين: الاشتراك (الخطّة ونهايتها وحالة التفعيل
     * وسقف الجودة) والكتالوج نفسه (القنوات والفئات كما يرسلها `player_api`).
     * تغيُّرها وحده يعني أنّ الكتالوج لم يعد صحيحاً — فيُعاد بناؤه. وثباتها
     * يعني ألّا شيء يُفعل، وهو الغالب: ردٌّ في مئتَي بايت بدل سحب مئتَي قناة
     * كلّ بضع دقائق لاكتشاف أنّ شيئاً لم يتغيّر.
     *
     * ⚠ كان الشقّ الأوّل وحده. فمن أضاف قناةً في اللوحة أو أوقفها أو أعاد
     *   ترتيب فئة لم يصل تعديلُه إلى جهازٍ واحد حتى تحلّ المزامنة الدوريّة.
     *
    ...
    data class Status(
        ...
        val subEnd: Long?,
        /**
         * ثوانٍ بين نبضتين، كما يقولها الخادم.
         *
         * الإيقاع بيده لا بيد التطبيق: لو ثقُل يوماً رفعناه من اللوحة، ولا
         * ننتظر إصداراً جديداً تُحدَّث إليه أجهزةُ الميدان — وأكثرها لا
         * يُحدَّث أبداً.
         */
        val pollSeconds: Int,
        /**
